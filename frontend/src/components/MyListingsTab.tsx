...
export function MyListingsTab({ 
  ...
  if (!isLoggedIn) {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
        ...
        <h3 className="text-xl text-gray-900 mb-2">Требуется авторизация</h3>
        <p className="text-gray-600 mb-6">Войдите в аккаунт, чтобы увидеть свои объявления и получить персональные рекомендации</p>
        <button
  ...
  const displayItems = subTab === 'services' ? userServices : userTasks;
  const emptyIcon = subTab === 'services' ? Briefcase : ListTodo;
  const emptyText = subTab === 'services' ? 'У вас пока нет размещенных услуг' : 'У вас пока нет размещенных задач';
    ...
    <div className="space-y-6">
      ...
      {displayItems.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayItems.map((item) => renderCard(item, subTab === 'services' ? 'service' : 'task'))}
        </div>
      ) : (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          {emptyIcon && React.createElement(emptyIcon, { className: 'w-16 h-16 text-gray-300 mx-auto mb-4' })}
          <p className="text-gray-500 mb-2">{emptyText}</p>
          <p className="text-sm text-gray-400">
            Создайте {subTab === 'services' ? 'услугу' : 'задачу'}, чтобы получить рекомендации
          </p>
        </div>
      )}
